...
class EmployeeIds(models.TransientModel):
    _inherit = 'hr.payslip.employees'

    currency_id = fields.Many2one("res.currency", string="Currency",related="structure_id.currency_id",store=True)
    show_rate=fields.Boolean(string="Show Rate",compute="_compute_show_rate",store=True)
    ...
    def _compute_show_rate(self):
        for record in self:
            if record.currency_id != self.env.company.currency_id:
                record.show_rate = True
            else:
                record.show_rate = False
